  ...
  const extraOptions = await inquirer.prompt([
    {
      ...
      name: "options",
      choices: Object.entries(EXTRA_OPTIONS.SELLING).map(([key, value]) => ({
  ...
  const maxPrice = await inquirer.prompt([
    {
      ...
      default: 0,
      validate: (value) =>
        /\d/.test(value) && parseFloat(value) <= MAX_PRICES[wallet.currency]
          ? true
  ...
  const extraOptions = await inquirer.prompt([
    {
      ...
      message: "Choose extra options",
      name: "options",
      choices: Object.entries(EXTRA_OPTIONS.BUYING).map(([key, value]) => ({
        name: TRANSLATION[key],
        value,
      })),
    },
