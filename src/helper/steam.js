...
const getFinalPrice = async (transactionId) => {
	...
	for (let i = 0; i < 3; i += 1) {
		response = await getRequest(
			`https://store.steampowered.com/checkout/getfinalprice/?count=1&transid=${transactionId}&purchasetype=self&microtxnid=-1&cart=${global.shoppingCartGID}&gidReplayOfTransID=-1`,
		);
...
const buyGames = async (config, ownedApps, ownedAppsRealCount, wallet) => {
	...
	if (config.usage !== "preview") {
		...
		const chunkSize =
			expectedChunkSize < minChunkSize ? minChunkSize : expectedChunkSize;
	...
	new Promise(async (resolve) => {
		...
		manager.getUserInventoryContents(
			...
			async (err, inventory) => {
				...
				console.log(
					`Found ${backgrounds.length} backgrounds, ${emoticons.length} emoticons, ${cards.length} cards, and ${boosters.length} boosters.`,
				);
